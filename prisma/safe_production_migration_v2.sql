-- Ensure OrderStatus has READY_TO_COLLECT
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t
    JOIN pg_enum e ON t.oid = e.enumtypid
    WHERE t.typname = 'OrderStatus' AND e.enumlabel = 'READY_TO_COLLECT'
  ) THEN
    ALTER TYPE "public"."OrderStatus" ADD VALUE 'READY_TO_COLLECT';
  END IF;
END $$;
...
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname='public' AND indexname='cart_items_userId_menuItemId_variantId_key'
  ) THEN
...
WITH base AS (
  SELECT id, LOWER(REGEXP_REPLACE(name, '[^a-zA-Z0-9\s_-]', '', 'g')) AS cleaned, "menuId"
...
-- Ensure no null slugs remain, fallback to id suffix
UPDATE "public"."menu_items" SET slug = 'item-' || RIGHT(id, 8) WHERE slug IS NULL;
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'categories_universityId_fkey'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'menu_item_availability_menuItemId_fkey'
  ) THEN
